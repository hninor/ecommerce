        ...
        when (intent) {
            ...
            is AddCardIntent.StringFieldChanged -> {
                when (intent.fieldType) {
                    AddCardFieldType.CARD_NUMBER -> reduceFields(
                        currentState.formFields.copy(
                            cardNumber = UiField(intent.fieldValue)
                        )
                    )

                    AddCardFieldType.CARD_HOLDER -> reduceFields(
                        currentState.formFields.copy(
                            cardHolder = UiField(intent.fieldValue)
                        )
                    )

                    AddCardFieldType.ADDRESS_LINE_1 -> reduceFields(
                        currentState.formFields.copy(
                            addressFirstLine = UiField(intent.fieldValue)
                        )
                    )

                    AddCardFieldType.ADDRESS_LINE_2 -> reduceFields(
                        currentState.formFields.copy(
                            addressSecondLine = UiField(intent.fieldValue)
                        )
                    )

                    AddCardFieldType.CVV_CODE -> reduceFields(
                        currentState.formFields.copy(
                            cvvCode = UiField(
                                intent.fieldValue
                            )
        ...
        if (error != null) {
            ...
            val updatedFields = when (fieldType) {
                ...
                AddCardFieldType.ADDRESS_LINE_2 -> {
                    currentFields.copy(
                        addressSecondLine = currentFields.addressSecondLine.copy(
                            error = uiError
                        )
                    )
                }
